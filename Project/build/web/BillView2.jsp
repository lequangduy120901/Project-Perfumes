...
<html>
    ...
    <body>
        <%
            ...
            ResultSet rs2 = (ResultSet) request.getAttribute("rs");
            int count = 0;
        %>
        <table border="1" style="width: 1000px; border: 1px solid black; background-color: #EEEEEE">
            ...
            <form action="ControllerHome?action=updateBill" method="POST">
                <td>
                    <input type="hidden" name="oid" value="<%=rsBill.getString(2)%>">
                    <select  name="status">
                        <%if (rs2.next()) {%>
                        <option value="<%=rs2.getInt(1)%>">
                            <%if (rs2.getInt(1) == 1) {%>Wait<%}%>
                            <%if (rs2.getInt(1) == 2) {%>Process<%}%>
                            <%if (rs2.getInt(1) == 3) {%>Done<%}%>
                        </option>
                            <%}%>
                            <%while (rsBill.next()) {%>
                            <%if (rsBill.getInt(7) != rs2.getInt(1)) {%>
                        <option value="<%=rsBill.getInt(7)%>">
                ...
                <td><a href="ControllerHome?action=updateBill&oid=<%=rsBill.getString(1)%>">Update</a></td>
